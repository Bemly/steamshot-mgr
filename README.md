...
| 全尺寸预览 | 主界面与导入 | 导入列表(转换压缩) |
|:---:|:---:|:---:|
| ![全尺寸预览](docs/20260822194902_1.jpg) | ![主界面与导入](docs/20260822194916_1.jpg) | ![导入列表](docs/20260822195053_1.jpg) |
...
| 导出 | **显式写**:仅写入用户选择的目标目录,全程不触碰 Steam 目录 |
...
- 入口:顶栏 **[导出…]**(位于导入右侧,需先在左侧选中一个游戏)。
- **依赖 ffmpeg**:点击后先在 PATH 中检测 ffmpeg;未安装则弹窗提示下载并加入环境变量(附一键打开下载页),缺少 AV1 编码器(需 `libsvtav1` 或 `libaom-av1`,gyan.dev full 版自带)同样提示。
- **输出位置**:自选目标目录,程序在其下创建**以游戏名命名的子文件夹**(NTFS 非法字符 `\ / : * ? " < > |` 自动转全角)。
- **格式与命名**:全部截图经 ffmpeg 转为 **AVIF**(CRF 25,约 60% 压缩率),文件名取截图时间戳 `YYYY-MM-DD-HH-MM-SS.avif`;同秒冲突自动顺延 1 秒,绝不覆盖。
- **文件时间**:导出后自动把文件的创建/修改时间设为文件名中的时间。
- **动态并发**:从 2 个 ffmpeg 进程起步,按 CPU 利用率(<90% 加、>98% 减)在 1 ~ 逻辑核心数之间自动伸缩,维持刚好满载。
- **进度与取消**:实时显示已完成/总数与当前文件,失败清单一目了然;[取消导出] 立即终止在跑任务并汇总已完成部分。
...
- **导出功能**需要 [ffmpeg](https://www.gyan.dev/ffmpeg/builds/)(full 版)且其目录已加入 `PATH` 环境变量
...
| 顶栏 [导出…] | 检测 ffmpeg → 选目标目录 → 批量导出 AVIF 到 游戏名\ 子文件夹 |
| 导出进度:[取消导出] | 终止在跑任务,保留已完成部分并汇总 |
| 主界面 `Ctrl+R` | 重新扫描截图 |
| 调整窗口大小 | 网格列数自适应 |
